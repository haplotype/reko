...
for j in ["pm90","pm95","pm99"]:
        fin = f"{j}/power2.susie.txt"
        df1 = pd.read_csv(fin, delim_whitespace=True, header=None)
        dfs.append(df1)
for j in ["pm90","pm95","pm99"]:
        fin = f"{j}/power2.koscreen.txt"
        df1 = pd.read_csv(fin, delim_whitespace=True, header=None)
        dfs.append(df1)
for j in ["pm90","pm95","pm99"]:
        fin = f"{j}/power2.reko.txt"
        df1 = pd.read_csv(fin, delim_whitespace=True, header=None)
        dfs.append(df1)
#dfs.append(df1)
...
base_colors=["green"]*3+["blue"]*3+["red"]*3
...
#mylabels = ["SuSiE","KoScreen", "ReKo"]*3
...
#ax2.legend(title='Blocks', loc='upper left', fontsize=10)
ax2.tick_params(axis='both', which='major', labelsize=16); 


handles, labels = ax1.get_legend_handles_labels()
leg=ax1.legend(handles,labels, ncol=3,
                loc='lower right', 
...
left_x = leg_pos.x0 + 0.1
print(leg_pos.x0,leg_pos.x1) 
print(leg_pos.y0,leg_pos.y1) 
column_width=(leg_pos.x1-leg_pos.x0)/3
plt.figtext(left_x + column_width*0.5, leg_pos.y1 + 0.0,
            'SuSiE', ha='center', fontweight='normal',fontsize=16)
plt.figtext(left_x + column_width*1.5, leg_pos.y1 + 0.0,
            'KnockoffScreen', ha='center', fontweight='normal',fontsize=16)
plt.figtext(left_x + column_width*2.5, leg_pos.y1 + 0.0,
            'ReKo', ha='center', fontweight='normal',fontsize=16)
